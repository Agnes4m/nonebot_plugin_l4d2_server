...
群组查询（如 `云`、`云 1`）输出的服务器列表图支持自定义背景，按以下步骤操作：

1. 找到插件的数据目录 `data/L4D2/`（与 `云.json` 等组文件同级）；
2. 在其中创建 `custom_backgrounds` 文件夹（插件启动时也会自动创建）；
3. 把你想要的背景图放入该文件夹，支持 `.jpg` / `.jpeg` / `.png` 格式；
4. 完成。下一次查询输出的图片即会使用新背景。

说明：

- **优先级**：`custom_backgrounds/` 里有图片时使用自定义背景；文件夹为空或图片被删除时，自动回落到插件内置的默认背景（`render/backgrounds/background.jpg`）；
- **选择规则**：文件夹中有多张图片时，按文件名排序取第一张，每次渲染使用同一张；只想用某张图时，删掉其余图片即可；
- **生效时机**：无需重启；替换同名图片会按文件修改时间自动重新加载；
